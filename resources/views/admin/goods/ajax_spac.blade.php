<table class="table table-bordered" id="goods_spec_table1">                                
    ...
    <tr>
        <td>{{$v->attr_name}}:</td>
        <td> 
                @foreach($v['attr_values'] as $ks=> $val)
                <button type="button" data-spec_id='{{$v->id}}' data-item_id='{{$val}}' class="btn ">
                  {{$val}}
                </button>                                                        
                        <img width="35" height="35" src="/admin/images/add-button.jpg" id="item_img_{$val}" onclick="GetUploadify3('{$val}');"/>    
                        <input type="hidden" name="item_img[{$ks}]" value="{$specImageList[$ks]}" />                                                      
                &nbsp;&nbsp;&nbsp;            
            @endforeach         
        </td>
    </tr>                                    
