    ...
    <div class="col-sm-8 blog-main">
        <div class="blog-post">
            <div style="display:inline-flex">
                    ...
                    @can('update',$posts)
                    <a style="margin: auto"  href="/posts/edit/{{$posts->id}}">
                        <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
                    </a>
                    @endcan
                    @can('delete',$posts)
                                                            <a style="margin: auto"  href="/posts/delete/{{$posts->id}}">
                        <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
                    </a>
                    @endcan
                                </div>

            <p class="blog-post-meta">May 14, 2017 by <a href="#">{{$posts->user->name}}</a></p>

            {!!$posts->content!!}
            <div>
             @if($posts->zan(\Auth::id())->exists())
             <a href="/posts/unzan/{{$posts->id}}" type="button" class="btn btn-primary btn-lg">取消赞</a>
             @else
              <a href="/posts/zan/{{$posts->id}}" type="button" class="btn btn-primary btn-lg">赞</a>
             @endif
            </div>
        </div>
